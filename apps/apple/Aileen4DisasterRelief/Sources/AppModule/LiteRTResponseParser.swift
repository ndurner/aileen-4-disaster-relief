...
enum LiteRTToolValue: Hashable, CustomStringConvertible {
    ...
    init?(jsonValue: Any) {
        ...
        case let string as String:
            self = .string(
                LiteRTResponseParser.normalizedQuotedTokenString(string)
            )
...
enum LiteRTResponseParser {
    ...
    fileprivate static func matchingQuotedToken(in raw: String, at index: String.Index) -> String? {
        textualQuotedTokens.first { raw[index...].hasPrefix($0) }
    }

    fileprivate static func normalizedQuotedTokenString(_ raw: String) -> String {
        textualQuotedTokens
